...
async def test_get_nearest_taxi(db_session: AsyncSession) -> None:
    taxi1 = await Taxi(x=1, y=12, callback_url="http://mockurl:8080").create(db_session)
    await Taxi(x=100, y=100, callback_url="http://mockurl:8080").create(db_session)
...
async def test_get_nearest_taxi_busy_taxis_return_none(
    ...
    await Taxi(
        x=1, y=12, status=TaxiStatus.BUSY, callback_url="http://mockurl:8080"
    ).create(db_session)
    await Taxi(
        x=100, y=100, status=TaxiStatus.BUSY, callback_url="http://mockurl:8080"
    ).create(db_session)
